...
DEFAULT_COUNTERS = {
    ...
    'http_requests_total': 'http.requests',  # Total number of HTTP requests made.
...
TRANSFORM_VALUE_SUMMARIES = {
    ...
    'http_request_duration_microseconds': 'http.request.duration',
}
...
DEFAULT_GAUGES = {
    # A metric with a constant '1' value labeled by major, minor, git version, git commit,
    # git tree state, build date, Go version, and compiler from which Kubernetes was built,
    # and platform on which it is running.
    'kubernetes_build_info': 'kubernetes.info',
...
class KubeSchedulerCheck(KubeLeaderElectionMixin, OpenMetricsBaseCheck):
    KUBE_SCHEDULER_NAME = "kube_scheduler"
    LEADER_ELECTION_CONFIG = {
        "namespace": "kube_scheduler",
        "record_kind": "endpoints",
        "record_name": "kube-scheduler",
        ...
        super(KubeSchedulerCheck, self).__init__(
            ...
            default_instances={
                "kube_scheduler": {
                    ...
                    'namespace': self.KUBE_SCHEDULER_NAME,
                    'metrics': [DEFAULT_COUNTERS, DEFAULT_HISTOGRAMS, NEW_1_14_HISTOGRAMS,
                                DEFAULT_SUMMARIES, DEFAULT_GAUGES, DEFAULT_GO_METRICS, DEPRECARED_SUMMARIES],
                }
            },
    ...
    def _filter_deprecated_metrics(self, metric, scraper_config):
        if metric.documentation.startswidth("(Deprecated)"):
            return
        self.submit_openmetric(self.MAPPING[metric.name], metric, scraper_config)
